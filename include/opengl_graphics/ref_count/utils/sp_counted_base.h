...
#include "opengl_graphics/ref_count/utils/non_copyable.h"
  ...
  public:
    ...
    bool add_ref_lock()
    {
      for (;;) {
        ...
        long cmp = std::atomic_compare_exchange_strong(
          &_use_count, &tmp, tmp + 1);
        if(cmp == tmp) {
          return true;
        }
      }
